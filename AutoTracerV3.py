...
thresh_close_btn = edge_btn = contour_iterations = edge_menu = contour_menu = save_name_entry = save_name_label = None
...
cc_calc_btn = cc_contour_check_btn = mag_status = int_calc_btn = crop_canvas = thresh_canvas = thresh_img = click_contours = click_index = click_tuple = contour_tuple = low_slide = up_slide = None
...
def crop_image(): ##Creates a menu to crop the image 
    global crop_menu, lower_crop, upper_crop, cropping_image, crop_canvas, crop_close, orig_resize
    ...
    crop_canvas.grid(row = 1, columnspan = 2)
...
def thresh_image(): ##Creates a menu to adjust the thresholds on the image
    global thresh_menu, lower_thresh, upper_thresh, thresh_panel_new, contour_iterations, thresh_panel_old, thresh_close_btn, low_slide, up_slide
    ...
    Label(thresh_menu, text="Please enter the lower threshold value", font= ("Helvetica", 12)).grid(row = 3, column = 0, sticky = "E") #Entry to adjust lower threshold
    ...
    Label(thresh_menu, text = "Please enter the higher threshold value", font= ("Helvetica", 12)).grid(row=5, column = 0, sticky = "E") #Entry to adjust upper threshold
...
def add_contours(iteration, lower_thresh, upper_thresh): ##Creates a menu to select contours
    global contour_menu, contour_input, lower_list, upper_list, tracing_image, list_label, save_csv_btn, undo_btn, thresh_canvas, thresh_img, click_contours
    ...
    thresh_canvas.grid(row=1, columnspan=5)
...
def add_upper(contours, contour_number): ##Adds the upper half of a contour to the upper list
    global upper_list, csvux, csvuy, tracing_img, csvlx, csvly, save_csv_btn, in_calc_btn, edge_close_btn, contour_close_btn, undo_btn, lower_list, save_name_entry, save_name_label
...
def add_lower(contours, contour_number): ##Adds the lower half of a contour to the lower list
    global lower_list, csvlx, csvly, tracing_img, contour_close_btn, in_calc_btn, edge_close_btn, csvux, csvuy, save_csv_btn, undo_btn, upper_list, save_name_entry, save_name_label
...
def save_csv_manual():
     ...
     #Resets buttons
        ...
        cbtn = thresh_btn = edge_btn = save_csv_btn = crop_status = thresh_status = contour_status = scale_ratio = scale_ratio_label = save_name_entry = save_name_label = None
...
def internal_calculations(path, ratio, lower_threshold, upper_threshold, crop_lower, crop_upper, scale_ratio, upper_csv, lower_list, lower_csv):
 ...
 #Polynomial Lower Trace
    ...
    #Find a way to improve polyfit
    
    #if len(c1) <1000:
    poly_fit = poly.polyfit(poly_x_list, poly_y_list, 6) #Creates a polynomial fit of 6th degree based on the data
    #elif len(c1)<2000:
        #poly_fit = poly.polyfit(poly_x_list, poly_y_list, 12)
    #else:
        #poly_fit = poly.polyfit(poly_x_list, poly_y_list, 18)
    #for i in range(6,20):
    #    poly_fit = poly.polyfit(poly_x_list, poly_y_list, i)
    #    poly_length = range(internal_img_width)
        #poly_y = np.polyval(poly_fit[::-1], poly_length)
